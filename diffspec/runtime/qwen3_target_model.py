...
class Qwen3Attention(nn.Module):
    ...
    def forward(
        ...
        is_draft: bool = False,
        init: bool = False,
        target_use_flash_prefill: bool = False,
        target_use_hybrid_tree_attn: bool = False,
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        present_key_value = (key_states, value_states) if return_kv else None
        ...
        if init:
            ...
            if target_use_flash_prefill:
                ...
                attn_output = torch.nn.functional.scaled_dot_product_attention(
                    ...
                    key_states,
                    value_states,
                    attn_mask=attention_mask.to(dtype=query_states.dtype) if attention_mask is not None else None,
            ...
            attn_weights = None
            if not (retrieve_attn_scores and is_last_layer):
                attn_weights = None
        ...
        if target_use_hybrid_tree_attn:
            ...
            current_out, weight = tree_attention_forward(
                q_flash,
                full_k[:, cache_len:, :, :],
                full_v[:, cache_len:, :, :],
                attention_mask,
            ...
            attn_output = attn_output.contiguous()
            attn_output = attn_output.reshape(*input_shape, -1).to(hidden_states.dtype)
            attn_output = self.o_proj(attn_output)
            attn_weights = None
        else:
...
class Qwen3Model(Qwen3PreTrainedModel):
    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        """Qwen3Model forward with DiffSpec extension arguments.

        Args:
            tree_attention_mask: Tree attention mask used during verification.
            return_kv: Return key/value tensors from decoder layers.
            is_draft: Mark the call as draft-model execution.
            init: Mark the prefill phase.
            target_use_flash_prefill: Use FlashAttention during target prefill.
            target_use_hybrid_tree_attn: Use hybrid tree attention for verification.
            retrieve_attn_scores: Return last-layer attention scores.
            best: Internal DiffSpec compatibility switch.
        """
        ...
        if tree_attention_mask is None and init and target_use_flash_prefill:
            ...
            if is_draft:
                ...
                tree_mask = torch.where(
                    tree_attention_mask == 0, torch.finfo(torch.float32).min, tree_attention_mask
                )
                zero_tensor = torch.tensor(0, dtype=torch.float32, device=inputs_embeds.device)
                tree_mask = torch.where(tree_mask == 1, zero_tensor, tree_mask)
        ...
        for idx, decoder_layer in enumerate(self.layers[: self.config.num_hidden_layers]):
            is_last_layer = idx == self.config.num_hidden_layers - 1
...
class Qwen3ForCausalLM(Qwen3PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        """Qwen3ForCausalLM forward with DiffSpec extension arguments.
