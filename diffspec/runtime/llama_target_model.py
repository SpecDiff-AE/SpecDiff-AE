...
from .attention_compat import FLASH_ATTN_AVAILABLE, flash_attn_func, flash_attn_with_kvcache
from .tree_attention import prefix_attention_forward, tree_attention_forward
...
class LlamaRotaryEmbedding_L31(nn.Module):
    def __init__(
        ...
        super().__init__()
        # Preserve constructor compatibility with older Transformers configs.
        self.rope_kwargs = {}
        if config is None:
            logger.warning_once(
            ...
            if target_use_hybrid_tree_attn:
                if retrieve_attn_scores and is_last_layer:
                    ...
                    if FLASH_ATTN_AVAILABLE:
                        ...
                        prefix_o, prefix_lse = prefix_attention_forward(
                            q_flash,
                            full_k[:, :cache_len, :, :],
                            full_v[:, :cache_len, :, :],
                            bsz,
                    ...
                    current_out, weight = tree_attention_forward(q_flash, 
                                                                full_k[:,cache_len:,:,:], 
                                                                full_v[:,cache_len:,:,:],
                                                    attention_mask, cache_len, prefix_lse, 
                                                    bsz, q_len, self.num_heads, self.head_dim)
                                    
                    attn_output = current_out
                    attn_output.mul_(1.0 - weight)
                    attn_output.add_(prefix_o.to(attn_output.dtype).mul_(weight))
                    attn_output = attn_output.contiguous()
                ...
                use_sdpa_tree = (
                    ...
                    and q_len <= 512
                    and not (retrieve_attn_scores and is_last_layer)
                )
                if use_sdpa_tree:
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # sumlist.append(elapsed)
        # if len(sumlist)  == 6000:
        #     sumlist = sumlist[2000:]
        #     print(f'attention took {sum(sumlist)} seconds, average time: {sum(sumlist) / len(sumlist)}')
        # end_time = time.time()
        # print("target attention time is", end_time - start_time)
        ...
        hidden_states = residual + hidden_states
        # end_time = time.time()
        # print("target FFN time is", end_time - start_time)
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        # embed positions
        if tree_attention_mask is None and init and target_use_flash_prefill:
            attention_mask = None
